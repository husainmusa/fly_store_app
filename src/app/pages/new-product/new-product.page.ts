    ...
    this.api.post('products/getByStoreId', param).subscribe((data: any) => {
      if (data && data.status === 200 && data.data && data.data.length) {
        for(let i = 0; i < this.parentId.length; i++){
          const item = data.data.filter(x => x.id === this.parentId[i]);
          this.dummyName.push(item[0].name);
        }
        this.parentName = this.dummyName.join(',');
      } 
    });
  }
  ...
  async openParent() {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data && data.data && data.role === 'selected') {
        ...
        this.parentName = data.data.name.join(',');
      }
    });
  }
