...
var OrderDetailPage = /** @class */ (function () {
    ...
    OrderDetailPage.prototype.getOrder = function () {
        ...
        this.api.post('orders/getById', param).subscribe(function (data) {
            ...
            if (data && data.status === 200 && data.data.length > 0) {
                ...
                // this.grandTotal = 0;
                _this.grandTotal = _this.empty(info.grand_total) ? "" : parseInt(info.grand_total);
                console.log("grandtotal", _this.grandTotal);
                _this.delivery_charge = _this.empty(info.delivery_charge) ? "" : parseInt(info.delivery_charge);
                _this.tax = _this.empty(info.tax) ? "" : parseInt(info.tax);
                _this.allTotal = _this.delivery_charge + _this.grandTotal + _this.tax;
                console.log("alltotal", _this.allTotal);
                var total_1 = 0;
                _this.orders.forEach(function (element) {
                    ...
                    var element_price = element.sell_price * element.quantiy;
                    // const price = element.sell_price === '0.00' ? parseFloat(element.original_price) : parseFloat(element.sell_price);
                    var items = '<div style="border-bottom:1px dashed lightgray;display:flex;flex-direction:row;justify-content:space-between;"> <p style="font-weight:bold">' + element.name + ' X ' + element.quantiy + '</p> <p style="font-weight:bold">' + _this.util.currecny + element_price + ' </p></div>';
                    _this.orderString.push(items);
                    var addon_price;
                    if (element.variations[0]) {
                        element.variations[0].items.forEach(function (addonElement) {
                            if (addonElement.discount == 0) {
                                addon_price = addonElement.price * addonElement.quantity;
                            }
                            else {
                                addon_price = (addonElement.price - ((addonElement.price / 100) * addonElement.discount)) * addonElement.quantity;
                            }
                            var addon = '<div style="border-bottom:1px dashed lightgray;display:flex;flex-direction:row;justify-content:space-between;"> <p style="font-weight:bold">' + addonElement.title + ' X ' + addonElement.quantity + '</p> <p style="font-weight:bold">' + _this.util.currecny + addon_price.toFixed(2) + ' </p></div>';
                            _this.orderString.push(addon);
                        });
                    }
                    // <div *ngIf="element.variations[0]"><p *ngFor="let addon of element.variations[0].items; let ol = index">'+ addon.title +'</p></div>
                    console.log("orderString", _this.orderString);
    ...
    OrderDetailPage.prototype.empty = function (str) {
    ...
    OrderDetailPage.prototype.printOrder = function () {
        ...
        var content = '<div style="padding: 20px;display: flex;flex-direction: column;"> <h2 style="text-align: center;">Flyvip Order Summary</h2> <p style="float: left;margin:0px;"><b>' + this.util.store.name + '</b></p> <p style="float: left;margin:0px;"><b> ' + this.userInfo.first_name + ' ' + this.userInfo.last_name + ' </b></p> <p style="float: left;margin:0px;">' + this.datetime + ' </p> </div>' + order
            + '<p style="border-bottom: 1px solid black;margin:10px 0px;"> <span style="float: left;font-weight: bold;">SubTotal</span> <span style="float: right;font-weight: bold;">' + this.util.currecny + this.grandTotal +
            '</span> </p> <br> <p style="border-bottom: 1px solid black;margin:10px 0px;"> <span style="float: left;font-weight: bold;">Delivery Charge</span> <span style="float: right;font-weight: bold;">' + this.util.currecny + this.delivery_charge +
            '</span> </p> <br> <p style="border-bottom: 1px solid black;margin:10px 0px;"> <span style="float: left;font-weight: bold;">Service Tax</span> <span style="float: right;font-weight: bold;">' + this.util.currecny + this.tax +
            '</span> </p> <br> <p style="border-bottom: 1px solid black;margin:10px 0px;"> <span style="float: left;font-weight: bold;">Total</span> <span style="float: right;font-weight: bold;">' + this.util.currecny + this.allTotal + '</span> </p>';
        console.log(content);
